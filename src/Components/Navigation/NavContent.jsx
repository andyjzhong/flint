  ...
  return (
    <>
      <Flex align="center" justify="space-between" className="nav-content__mobile" {...props}>
        ...
        >
          <Flex>
            <Button mx='2' as="a" href="login" colorScheme="blue" display={isUserLoggedIn ? 'none' : 'flex'}>
              Sign In
            </Button>
            <Button 
              as="a" 
  ...
  return (
    <Flex className="nav-content__desktop" align="center" {...props} style={{position: "relative"}}>
      ...
      <HStack spacing="8" minW="240px" style={{position: "absolute", right: "0"}}>
        <Box as="a" href="/logout" color={mode('blue.600', 'blue.300')} fontWeight="bold" display={isUserLoggedIn ? 'block' : 'none'}>
          Sign out
        </Box>
        <Box as="a" href="login" color={mode('blue.600', 'blue.300')} fontWeight="bold" display={isUserLoggedIn ? 'none' : 'block'}>
          Sign in
        </Box>
        <Button as="a" href="signup" colorScheme="blue" fontWeight="bold" display={isUserLoggedIn ? 'none' : 'flex'}>
          Sign up
        </Button>
        <Menu>
            <MenuButton as={Avatar} size={"md"} cursor="pointer" src={localStorage.getItem('profilePicURL')} border={"2px solid #2B6CB0"} display={isUserLoggedIn ? 'block' : 'none'}>
            </MenuButton>
            <MenuList>
